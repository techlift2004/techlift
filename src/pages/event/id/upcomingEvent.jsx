...
export default function CardComponent() {
  ...
  return (
    <div className="flex flex-col gap-10 items-start pt-16 px-5 md:px-10 pb-20">
      ...
      {events.length === 0 ? (
        <p className="font-Nunito text-[18px] text-gray-text">No upcoming</p>
      ) : (
          ...
          {events.map((member, index) => (
            <Card key={index} className="w-full h-full rounded-2xl overflow-hidden shadow-md transition-transform hover:scale-[1.02] duration-300">
              <div className="flex flex-col items-start h-full">
                <div className="w-full h-[220px] sm:h-[280px] lg:h-[320px] bg-gray-100 flex items-center justify-center">
                  <img
                    src={member.image}
                    alt={member.name}
                ...
                <div className="flex flex-col justify-between items-center pt-3 gap-5 flex-grow">
                  <div className="flex flex-col items-start px-4 sm:px-5 gap-2 pb-6 pt-3 w-full">
                    ...
                    {member.link ? (
                      <Button asChild className="bg-button-background text-[14px] sm:text-[16px] lg:text-[18px] font-poppin font-medium py-3 sm:py-4 mt-4 w-full sm:w-auto">
                        <a href={member.link} target="_blank" rel="noopener noreferrer">
                          Register
                        </a>
                      </Button>
                    ) : (
                      <Button disabled className="bg-button-background text-[14px] sm:text-[16px] lg:text-[18px] font-poppin font-medium py-3 sm:py-4 mt-4 w-full sm:w-auto">
                        Register
                      </Button>
                    )}
                  </div>
