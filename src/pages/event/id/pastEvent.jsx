...
export default function CardComponent() {
  ...
  return (
    <div className='flex flex-col gap-8 items-start pt-16 px-5 md:px-10 pb-20'>
      ...
      {events.length === 0 ? (
        <p className='font-Nunito text-[18px] text-gray-text'>No past event yet</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full">
          {events.map((member, index) => (
            <Card key={index} className="w-full h-full rounded-2xl overflow-hidden shadow-md">
              <div className="flex flex-col items-start h-full">
                <div className="w-full h-[220px] sm:h-[300px] bg-gray-100 flex items-center justify-center">
                  <img
                    src={member.image}
                    alt={member.name}
